...
//    });
 ...
 // Calendar plugins
 var rutaUp = document.getElementById('ruta').innerHTML;
 var idRuta = document.getElementById('idRuta').value;
 rutaUp = rutaUp.replace('id',idRuta);

 var arraydatos = fetch(rutaUp).then(data => data.json());
 arraydatos.then(data => VerDatos(data));

 const VerDatos = (data) => {
    export var ququ = data.id;
 }
 console.log(ququ);
...
// }
 ...
 function updateEvent(eventData) {
   ...
   '<input name="DiaProgramacion[]" type="hidden" value="'+diaEv+'">');
       ...
       //   document.getElementById('bton').removeAttribute("hidden");
         let i;
           ...
           var hora = $('.fc-event-main-frame')[0].innerText.split('\n',1)[0];
               ...
               if(data[i].Program_Dia == fecha && horario == hora){
